...
function buildLambda(zipData, name, callback){
	...
	console.log(`Files before install:\n\t${ fs.readdirSync(lambdaFolder, { withFileTypes: true }).map(dirent => dirent.name).join('\n\t') }\n`);

				console.log("Installing...");

				process.env.HOME = lambdaFolder;

				startProcess('npm install --verbose', ()=>{ // --no-progress
				
	console.log(`Files after install:\n\t${ fs.readdirSync(lambdaFolder, { withFileTypes: true }).map(dirent => dirent.name).join('\n\t') }\n`);
